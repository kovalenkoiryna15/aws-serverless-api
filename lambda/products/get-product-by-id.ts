...
export const getProductById = async (event: APIGatewayProxyEvent) => {
  ...
  product = availableProducts.find(({ id }) => id === event.pathParameters?.product_id);  

  if (!product) {
    return response(404, `Not Found. There is no product with id ${event.pathParameters?.product_id}`);
  }
